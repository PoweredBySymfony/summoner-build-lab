...
type RecentMatch = PlayerSearchPayload["recentMatches"][number];

type GenerateSeriesFromMatch = (matchId: string, matchIndex: number) => void;

const SnapshotGenerationNotice = ({
  generationResult,
  generationError,
}: {
  generationResult: GeneratedMatchPuzzleResponse | null;
  ...
  <>
    {isSnapshotGenerationFailure(generationResult) ? (
      <div className="mt-4 rounded-2xl border border-border/60 bg-background/60 px-4 py-3 text-sm text-muted-foreground">
        <p>{getGenerationFailureMessage(generationResult)}</p>
        <p className="mt-2">
...
const PlayerProfileHero = ({
  ...
  data: PlayerSearchPayload | undefined;
  riotId: string | undefined;
  profileIconUrl: string | null;
  canGenerateLatest: boolean;
    ...
    <div className="grid gap-4 lg:grid-cols-[minmax(0,1.05fr)_minmax(320px,0.95fr)] lg:items-start">
      <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-start sm:gap-4">
        ...
        <div className="min-w-0">
          ...
          <SnapshotGenerationNotice generationResult={generationResult} generationError={generationError} />
        </div>
      </div>
...
const CompactStatsGrid = ({ compactStats }: { compactStats: ReturnType<typeof buildCompactStats> }) => (
...
const AccountOverview = ({ data }: { data: PlayerSearchPayload }) => (
...
const MostPlayedChampions = ({ data }: { data: PlayerSearchPayload }) => (
  <div className="glass-surface rounded-2xl p-4">
...
const MatchItems = ({ match }: { match: RecentMatch }) => (
...
const RecentMatchRow = ({
  ...
  onGenerate: GenerateSeriesFromMatch;
}) => (
      ...
      <div className="flex xl:justify-end">
        {canGenerate ? (
          <button
            ...
            onClick={() => onGenerate(match.matchId, matchIndex)}
            disabled={selectedMatchId === match.matchId || isActionPending}
          >
...
const PlayerProfileContent = ({
  ...
  data: PlayerSearchPayload;
  compactStats: ReturnType<typeof buildCompactStats>;
  canLoadMoreMatches: boolean;
  isFetching: boolean;
  canGenerate: boolean;
  selectedMatchId: string | null;
  ...
  <>
    <CompactStatsGrid compactStats={compactStats} />
    <section className="grid gap-4 xl:grid-cols-[320px_minmax(0,1fr)]">
      <div className="space-y-4">
        <AccountOverview data={data} />
        <MostPlayedChampions data={data} />
      </div>
      <RecentMatchesPanel
        data={data}
...
const PlayerProfile = () => {
  ...
  const isActionPending = importRecentMatches.isPending || generateMatchSeries.isPending;
  const canGenerate = Boolean(user);
  const canGenerateLatest = Boolean(user && data?.recentMatches[0] && !isActionPending);
  ...
  const generateSeriesFromLatestMatch = () => {
    ...
    <div className="min-h-screen bg-background pb-10 pt-20">
      <div className="container mx-auto max-w-[1700px] space-y-5 px-4">
        <PlayerProfileHero
          ...
          canGenerateLatest={canGenerateLatest}
          onGenerateLatest={generateSeriesFromLatestMatch}
          generationResult={generationResult}
          generationError={generationError}
        />

        <ProfileStatusPanels isLoading={isLoading} error={error} />
        ...
        {data ? (
          <PlayerProfileContent
            ...
            selectedMatchId={selectedMatchId}
            isActionPending={isActionPending}
            onGenerate={(matchId, matchIndex) => {
              void generateSeriesFromMatch(matchId, matchIndex);
            }}
            onLoadMore={() => setMatchFetchCount((current) => Math.min(20, current + 5))}
          />
        ) : null}
      </div>
    </div>
