...
function mapItem(row: ItemRow): GameItem {
  return {
    ...
    buildsFrom: Array.isArray(row.buildsFrom) ? (row.buildsFrom as string[]).map(String) : [],
    buildsInto: Array.isArray(row.buildsInto) ? (row.buildsInto as string[]).map(String) : [],
    isBoots: row.isBoots,
...
function mapChampion(row: ChampionRow): ChampionView {
  return {
    ...
    image: row.image,
    roles: [row.rolePrimary, row.roleSecondary].filter(Boolean),
    tags: Array.isArray(row.tags) ? (row.tags as string[]) : [],
    stats: row.stats && typeof row.stats === "object" && !Array.isArray(row.stats) ? (row.stats as Record<string, unknown>) : {},
    patch: row.patch,
    isActive: row.isActive,
